...
solvcon's canonical repository is `solvcon/solvcon` (the `upstream`
remote in a typical clone); its integration branch is `master`.
Contributors usually push their working branch to a personal fork
remote (e.g. `yyc`) and open the PR against `upstream/master`. Confirm
the remotes with `git remote -v` if unsure.
...
1. **Confirm scope with the user.** Ask directly when unclear from
   ...
   - Which remote/branch is the PR base? (default: `upstream/master`)

2. **Verify branch state.** Treat `upstream/master` as the base (adjust
   if the user named a different base). Run in parallel:
   - `git status --porcelain` -- check for staged or unstaged changes
     and untracked files.
   - `git log --oneline upstream/master..HEAD` -- list the commits the
     PR will carry.
   - `git diff --stat $(git merge-base upstream/master HEAD)..HEAD` --
     summarize the diff against the merge base.
   - `git rev-parse --abbrev-ref --symbolic-full-name @{u}` -- check if
     the branch tracks a remote (typically a personal fork).
   ...
   If the branch has no commits ahead of `upstream/master`, abort --
   the PR would be empty.

   If the branch is not pushed (or is behind its fork remote), push
...
4. **Open the PR.** Load the approved title and body into shell
   ...
   ready for review. Adjust `--base` if the user named a different
   integration branch. Do not append a second issue-reference footer
   here; the body from step 3 already has one. The `trap` ensures the
   temp file is removed on any exit path. If the approved body would
   itself contain the literal token `BODY` on its own line, swap the
   delimiter to something unique (e.g. `BODY_EOF_811`).
